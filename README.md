...
- ✅ 四维纯白空间：绿色神经元（胞体 + 内核 + 柔光）、弯曲连接、信号光点流动；拖拽移动、滚轮缩放、右键平移
- ✅ 点击神经元 → 编辑名称 / 角色 / 系统提示词 / 所属群组；复制 / 删除
- ✅ **连线模式**：依次点击两个神经元建立连接；点击连接 → 编辑关系类型（指令/批判/总结/投票/竞争/自定义）、方向（双向/单向）、权重
- ✅ 系统状态光谱：简单 / 有序 / 混沌边缘 / 混沌 / 无序 / 自定义；拓扑预设；时间轴（第 4 维）
- ✅ **真实 AI 对话**：OpenAI 兼容（OpenAI/DeepSeek/通义/Moonshot）+ 本地 Ollama；零依赖本地代理；平台只收集消息并标注元信息，**取舍完全交给模型**
- ✅ **多层级发散记忆库**：个人仅自己可读、共同双方可读、群组（含子组）成员可读；事件自动记录；推论由模型生成（🧠 整理记忆）；衰减；注入下一轮提示词；个人记忆可「↑群组」固化上升
- ✅ **涌现观察**：参与/共识/新颖/分工/关系多样性 + 综合涌现分；运行历史回看
- ✅ **涌现场景穷举**：辩论法庭/软件公司/科研小组/市场 4 个角色社会模板；自动生成变体批量运行；按涌现分排名；一键载入画布
- ✅ **嵌套群组**：可嵌套、成员归属、群组色环标识
- ✅ **新人进入体验**：新 AI 进入环境会自我介绍并记录初始印象
- ✅ 项目**导出/导入**（JSON 快照）
- ⏳ 未来：时间分支、进化选择压力、Hebbian 权重、相变图（见 PLAN.md）
